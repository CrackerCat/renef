...
std::string MemDumpCommand::get_name() const {
    return "md";
}

std::string MemDumpCommand::get_description() const {
    return "Memory dump/disassemble at address (md <addr> <size> [-d])";
}

CommandResult MemDumpCommand::dispatch(int client_fd, const char* cmd_buffer, size_t /* cmd_size */) {
