...
std::string get_lua_code(char* file_path){
    std::string lua_code;
...
std::string LoadScriptCommand::get_name() const {
    return "l";
}

std::string LoadScriptCommand::get_description() const {
    return "Load and execute a Lua script in the target process.";
}

CommandResult LoadScriptCommand::dispatch(int client_fd, const char* cmd_buffer, size_t cmd_size) {
