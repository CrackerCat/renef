...
std::string WatchCommand::get_name() const {
    return "watch";
}

std::string WatchCommand::get_description() const {
    return "Watch hook output in real-time (Ctrl+C to stop).";
}

CommandResult WatchCommand::dispatch(int client_fd, const char* cmd_buffer, size_t cmd_size) {
